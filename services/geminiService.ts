...
async function withRetry<T>(fn: () => Promise<T>, attempts = 3, delay = 1000): Promise<T> {
  try {
    ...
    if (attempts <= 1 || e?.status === 400) throw e;
...
function getAI() {
  if (!API_KEY) throw new Error("VITE_GEMINI_API_KEY is not set. Add it to .env.local");
...
): Promise<{ isSafe: boolean; score: number; summary: string; issues: string[] }> => {
  try {
...
export const analyzeHazard = async (
  imageBase64: string
): Promise<{ type: string; severity: 'Low' | 'Medium' | 'High' | 'Critical'; description: string }> => {
  try {
    return await withRetry(async () => {
      const ai = getAI();
      const response = await ai.models.generateContent({
...
export const detectBiddingWar = async (
  fileData: string,
  mimeType: string = 'image/jpeg'
): Promise<{ isIllegal: boolean; evidence: string }> => {
  try {
    return await withRetry(async () => {
      const ai = getAI();
      const response = await ai.models.generateContent({
...
export const analyzeEPC = async (
  fileData: string,
  mimeType: string = 'image/jpeg'
): Promise<{ score: string; comfort: number; compliance: boolean; summary: string }> => {
  try {
    return await withRetry(async () => {
      const ai = getAI();
      const response = await ai.models.generateContent({
...
export const analyzeRentIncrease = async (
  currentRent: number,
  newRent: number,
  location: string
): Promise<{ isFair: boolean; advice: string; letter: string }> => {
  try {
    return await withRetry(async () => {
      const ai = getAI();
      const response = await ai.models.generateContent({
...
export const generateDisputeLetter = async (
  ...
  evidence: string,
  amount: number
): Promise<{ advice: string; letter: string }> => {
  try {
    return await withRetry(async () => {
      const ai = getAI();
      const response = await ai.models.generateContent({
...
export const translateLandlordSpeak = async (
  text: string
): Promise<{ translation: string; legalStanding: string }> => {
  try {
    return await withRetry(async () => {
      const ai = getAI();
      const response = await ai.models.generateContent({
        model: 'gemini-3-flash-preview',
        contents: `Analyze this text from a landlord: "${text}".
